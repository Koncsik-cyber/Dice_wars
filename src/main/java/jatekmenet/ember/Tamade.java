...
public class Tamade {
    public void tamade(char valasz,int x,int y, int z){
        if (valasz == 'i') new MitTamadhatEmber(x,y,z);
        else new MelyikTamdaja();
    }
    public void inditETamadast(int valasz, int honnanX, int honnanY, int honnanZ, int hovaX, int hovaY, int hovaZ){
        if (FoMenuBeallitas.isPalyaTipus()) {
            if (valasz == 1 || valasz == 2 || valasz == 3 || valasz == 4) {
                ...
                tamad.tamdasIndul(honnanX, honnanY, honnanZ, hovaX, hovaY, hovaZ, true);
            } else new MelyikTamdaja();
        }else {
            if (valasz == 1 || valasz == 2 || valasz == 3 || valasz == 4 || valasz == 5 || valasz == 6) {
                ...
                System.out.println(new MentesAdatKezeles().MentesString("========>X<========"));
                Tamadas tamad = new Tamadas();
                tamad.tamdasIndul(honnanX, honnanY, honnanZ, hovaX, hovaY, hovaZ, false);
            } else new MelyikTamdaja();
        }
    }
}
